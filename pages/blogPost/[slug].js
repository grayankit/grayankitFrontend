      ...
      {props.data.data.map((item) => {
        return (
          <section className="text-gray-600">
            <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-center mt-4">
              {item.attributes.Title}
            </h1>
            {<div dangerouslySetInnerHTML={createMarkup(item.attributes.Content)} className="text-center text-lg"></div>}
          </section>
        );
      })}
    </div>
